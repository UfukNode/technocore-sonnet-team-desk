...
/**
 * Pairs every accepted receipt in a room export with the proposal holding its word.
 *
 * A receipt names the request it answers but does not repeat the word, so the two
 * have to be read together to recover what a team actually wrote. Only the referee
 * counts as a source of acceptance, and a receipt naming a request no proposal
 * claims is dropped rather than guessed at.
 */
function acceptedWords(lines, refereeDid) {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      record = parseRecord(message.text);
    } catch { continue; }
    if (!record) continue;
    if (record.type === "sonnet.word.v1") {
      if (typeof record.request_id === "string" && WORD_RE.test(String(record.word))) proposals.set(record.request_id, String(record.word));
      continue;
    }
    if (message.from !== refereeDid || receiptStatus(record) !== "accepted") continue;
    const reference = referencedRequestId(record);
    if (typeof reference !== "string" || !proposals.has(reference)) continue;
    words.push(proposals.get(reference));
  }
  return words;
}
